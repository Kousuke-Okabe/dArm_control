...
void Subscribe_TCP_Command(const std_msgs::Float64MultiArray &TCP_cmd){
  ...
  eta_command_angle = (double)TCP_cmd.data[2];
}

int main(int argc, char* argv[]){
  ROS_INFO("Start tcp_control node");
  ...
  double Kp_r = nh.param<double>("Position_Gain",1);
  double Kp_eta = nh.param<double>("Orientation_Gain",1);

  // ROS_INFO("Set cycle");
  // 周期設定
  int Fs = nh.param<int>("TCP_CONTROL_Frequency",100);
  ...
  while(ros::ok()){
    ...
    q_command_velocity = q_command_velocity + Jp *( Kp_r*(r_command_position - r_present_position)/Ts );
    q_command_velocity = q_command_velocity + (I-Jp*J)*Jeta*Kp_eta*(eta_command_angle-eta_present_angle)/Ts;
